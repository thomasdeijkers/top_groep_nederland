...
class PayrollCalculationTests(unittest.TestCase):
    ...
    def test_week_workbook_rows_link_to_timesheets(self):
        ...
        self.assertIn("/dashboard/relations?tab=candidates&edit={{ row.relation_id }}&return_to=", template)
...
class PayrollPeriodStructureTests(unittest.TestCase):
    ...
    def test_payroll_periods_can_only_be_archived_from_dashboard(self):
        ...
        self.assertIn('action="/api/periods/{{ selected_payroll_period.id }}/archive"', template)
        self.assertNotIn('action="/api/periods/{{ period.id }}/archive"', template)
...
class CompletePeriodTimesheetImportTests(unittest.TestCase):
    ...
    def test_payroll_payment_flow_has_workbook_tabs_and_actions(self):
        ...
        router_source = Path("apps/dashboard/router.py").read_text(encoding="utf-8-sig")
        records_source = Path("apps/dashboard/records.py").read_text(encoding="utf-8-sig")
        dashboard_js = Path("apps/dashboard/static/dashboard.js").read_text(encoding="utf-8-sig")
        dashboard_css = Path("apps/dashboard/static/dashboard.css").read_text(encoding="utf-8-sig")
        data_store_source = Path("apps/dashboard/data_store.py").read_text(encoding="utf-8-sig")
        payment_status_migration = Path("migrations/049_payroll_week_input_payment_status.sql").read_text(encoding="utf-8-sig")
        ...
        self.assertIn('data-payroll-summary-total="Uit te betalen"', template)
        self.assertIn('data-payroll-summary-total="Uitbetaald"', template)
        self.assertIn("Loon te valideren", template)
        self.assertIn("Uit te betalen loon", template)
        self.assertIn("payroll_flow_summary.validate_total", template)
        self.assertIn("overview_data.payroll_flow.payable_total", template)
        self.assertIn("def get_payroll_flow_summary", records_source)
        self.assertIn("def _payroll_effective_status_sql", records_source)
        self.assertIn('_payroll_effective_status_sql("i")', records_source)
        self.assertIn("NULLIF({payroll_status}, '')", records_source)
        self.assertIn("WHEN {status} IN ('uit_te_betalen', 'uitbetaald') THEN {status}", records_source)
        self.assertIn("WHEN {payroll_status} IN ('uit_te_betalen', 'uitbetaald') THEN {payroll_status}", records_source)
        self.assertIn("payroll_status = %s", records_source)
        self.assertIn('period["payroll_flow"]', records_source)
        self.assertIn("period-flow-mini", template)
        self.assertIn("period.payroll_flow.payable_total", template)
        self.assertIn("049_payroll_week_input_payment_status.sql", data_store_source)
        self.assertIn("ADD COLUMN IF NOT EXISTS payroll_status", payment_status_migration)
        self.assertIn("Uitbetalen", template)
        self.assertIn("Uitbetaald", template)
        self.assertIn("Terug naar loonregels", template)
        self.assertIn("Terug naar uit te betalen", template)
        self.assertIn("data-payroll-block-modal", template)
        self.assertIn("data-block-fields", template)
        self.assertIn("Medewerker-inrichting mist: geldige periode-inrichting, contracturen, netto basisloon 40 uur, bruto uurloon, fase, pensioenregeling", records_source)
        self.assertIn("return_to={{ ('/dashboard/periods?period=' ~ selected_payroll_period.id ~ '#periode-verloning')|urlencode }}", template)
        self.assertIn('name="return_to" value="{{ relation_return_url or \'\' }}"', template)
        self.assertIn("relation_return_url", router_source)
        self.assertIn("def _safe_dashboard_return_url", router_source)
        self.assertNotIn(">Geen fase</option>", template)
        self.assertNotIn(">Geen pensioen</option>", template)
        self.assertNotIn(">Geen inregeling</option>", template)
        self.assertIn("dashboard-logout--nav", template)
        self.assertIn("period-status-tile", template)
        self.assertIn("<th>Loonflow</th>", template)
        self.assertNotIn("<th>Opmerkingen</th>", template)
        self.assertNotIn("period-remark-tile", template)
        self.assertIn("completion_note", records_source)
        self.assertIn("def _attach_period_list_notes", records_source)
        self.assertIn("data-source-value", template)
        self.assertIn("Origineel urenbriefje", template)
        self.assertIn("Override actief", template)
        self.assertIn("payroll-cell-input--overridden", dashboard_css)
        self.assertIn("setSourceMeta", dashboard_js)
        self.assertIn("original_value: sourceValue", dashboard_js)
        ...
        self.assertIn('"loon_berekenen"', records_source)
        self.assertIn('"uit_te_betalen"', records_source)
        self.assertIn('"uitbetaald"', records_source)
        self.assertIn("{effective_status} AS status", records_source)
        ...
        self.assertIn("aggregated_totals = aggregate_week_sheet_totals(payment_source_tabs)", calculations_source)
        self.assertIn("tab_is_editable = tab.kind == 'week' and not selected_payroll_period.is_locked_for_payment", template)
        self.assertIn("Alleen weektabs zijn bewerkbaar", template)
        self.assertIn('"loon_berekenen": "Loon berekenen"', records_source)
        self.assertIn('.get(result.get("status"), "")', router_source)
        self.assertIn("recalculatePaymentSummary", dashboard_js)
        self.assertIn(".payroll-col-net-amount", dashboard_js)
        self.assertIn("openPayrollBlockModal", dashboard_js)
        self.assertIn("blockerItemsFromButton", dashboard_js)
        self.assertIn("payroll-block-dialog", dashboard_css)
...
class TimesheetCandidateValidationTests(unittest.TestCase):
    ...
    def test_payroll_employee_settings_and_balances_are_editable_with_audit(self):
        ...
        router_source = Path("apps/dashboard/router.py").read_text(encoding="utf-8-sig")
        records_source = Path("apps/dashboard/records.py").read_text(encoding="utf-8-sig")
        relations_source = Path("apps/dashboard/relations.py").read_text(encoding="utf-8-sig")
        script = Path("apps/dashboard/static/dashboard.js").read_text(encoding="utf-8-sig")
        stylesheet = Path("apps/dashboard/static/dashboard.css").read_text(encoding="utf-8-sig")

        self.assertIn("/api/payroll/employee-arrangements/{{ arrangement.id }}", template)
        self.assertIn('name="payroll_net_base_40h"', template)
        ...
        self.assertIn("payroll_net_base_40h: str = Form(\"\")", router_source)
        self.assertIn("\"payroll_net_base_40h\": \"netto basis 40 uur\"", router_source)
        ...
        self.assertIn("def _sync_relation_payroll_arrangement", relations_source)
        self.assertIn("net_base_40h = %s", relations_source)
        self.assertIn("recalculate_open_payroll_for_relation(relation_id)", relations_source)
        ...
        self.assertIn("min-width: 132px", stylesheet)
        self.assertNotIn(".payroll-workbook-tab small:first-of-type:nth-last-of-type(2)", stylesheet)
...
class DashboardContextSafetyTests(unittest.TestCase):
    ...
    def test_payroll_period_controls_only_use_active_validation_rows(self):
        ...
        self.assertIn("effective_status = _payroll_effective_status_sql(alias)", records_source)
        self.assertIn("AND {effective_status} = ANY(%s)", records_source)
