...
class CompletePeriodTimesheetImportTests(unittest.TestCase):
    ...
    def test_payroll_payment_flow_has_workbook_tabs_and_actions(self):
        ...
        self.assertIn("COALESCE(NULLIF(i.payroll_status, ''), NULLIF(i.status, ''), 'loon_berekenen')", records_source)
        self.assertIn("THEN LOWER(REPLACE(COALESCE(i.status, ''), ' ', '_'))", records_source)
